...
uv add --dev cmake scikit-build-core
...
CUDACXX=/usr/local/cuda/bin/nvcc CMAKE_ARGS="-DGGML_CUDA=on" uv pip install llama-cpp-python
```
...
uv run python -c "from transformers import AutoConfig; print(AutoConfig.from_pretrained('./model').architectures)"

# Use appropriate conversion script
# For most models:
uv run python convert_hf_to_gguf.py ./model --outfile model.gguf
...
CUDA_VISIBLE_DEVICES="" uv run python convert_hf_to_gguf.py ./model --outfile model.gguf

# Or use low memory mode
uv run python convert_hf_to_gguf.py ./model --outfile model.gguf --outtype f16
```
