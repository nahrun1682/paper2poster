...
uv run python convert_hf_to_gguf.py ./path/to/model --outfile model-f16.gguf
...
```bash
# 1. Download model
uv run huggingface-cli download meta-llama/Llama-3.1-8B --local-dir ./llama-3.1-8b

# 2. Convert to GGUF (FP16)
uv run python convert_hf_to_gguf.py ./llama-3.1-8b \
...
```bash
# 1. Convert to GGUF
uv run python convert_hf_to_gguf.py ./model --outfile model-f16.gguf
