...
1) 論文・研究報告書のPDFを `data/paper/` に配置する。

2) 依存関係とブラウザ実体をセットアップする。

    uv sync
    uv run playwright install chromium

3) VS Code のエージェントチャットで、次のように依頼してポスター作成フェーズを開始する。

    paper-to-poster-drafterスキルを使ってdata/paperにある研究報告書からポスター作成フェーズを開始して

上記 1) 2) 3) が完了していれば、Playwright視覚確認を含むポスター生成フローを再現できる。
